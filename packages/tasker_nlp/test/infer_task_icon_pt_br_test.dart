...
void main() {
  group('inferTaskIconPTBR', () {
    ...
    test('lazer → leisure', () {
      final r = inferTaskIconPTBR('dia de lazer amanhã');
      expect(r.iconKey, 'leisure');
    });

    test('cinema → leisure', () {
      final r = inferTaskIconPTBR('ir ao cinema com os amigos sábado à noite');
      expect(r.iconKey, 'leisure');
    });

    test('sair com amigos → leisure', () {
      final r = inferTaskIconPTBR('sair com amigos no bar hoje');
      expect(r.iconKey, 'leisure');
    });

    test('netflix → leisure', () {
      final r = inferTaskIconPTBR('maratonar série no netflix hoje à noite');
      expect(r.iconKey, 'leisure');
    });

    test('videogame → leisure', () {
      final r = inferTaskIconPTBR('jogar videogame no playstation sábado');
      expect(r.iconKey, 'leisure');
    });
  });
}
